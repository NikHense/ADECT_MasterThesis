...
eps_list = list(np.arange(0.1, 7.2, 0.5)) # initial parameters
min_samples_list = list(range(25, 301, 25)) # initial parameters
...
param_list = [(eps, min_samples) for eps in eps_list
              for min_samples in min_samples_list]

# Create a Pool object
with Pool() as pool:
    # Compute DBSCAN for all parameter combinations in parallel
    results = pool.map(dbscan_cluster, param_list)
    pool.close()
    pool.join()
...
best_params = (best_row['eps'], best_row['min_samples'], best_row['n_clusters'], best_row['n_noise'])
best_score = best_row['score']
print(f'Best parameter combination: eps={best_params[0]:.6f}, min_samples={best_params[1]}, n_clusters={best_params[2]}, n_noise={best_params[3]}, score={best_score:.3f}')

# Save the best parameters for later use
eps = float(f"{best_params[0]:.3f}")
min_samples = int(f"{best_params[1]:d}")
...
top_30 = sorted(zip(scores_db, param_list), reverse=True)[:30]
top_30

# Plot Top_30
sns.set_style('whitegrid')
plt.figure(figsize=(10, 10))
plt.scatter([x[1][0] for x in top_30], [x[1][1] for x in top_30],
            c=[x[0] for x in top_30], cmap='viridis')
plt.colorbar()
plt.xlabel('eps')
plt.ylabel('min_samples')
plt.title('Top 30 DBSCAN scores (inital grid)')
plt.show()
