...
data_auto = pd.merge(data_auto, data_normal[['INDEX', 'y_1']], on='INDEX', how='left')
...
X = input_auto.drop(['y_1'], axis=1)
y = input_auto['y_1']

# Scaling the data
scaler = StandardScaler()
X = scaler.fit_transform(X)
X = pd.DataFrame(X, columns=input_auto.columns.drop(['y_1']))
...
print(f'The test data set has {sorted(Counter(y_test).items())[0][1]} records for the majority class and {sorted(Counter(y_test).items())[1][1]} records for the minority class.')
