...
data_auto = pd.merge(data_auto, data_normal[['INDEX', 'y_2']], on='INDEX', how='left')
...
print(f'The training dataset has {sorted(Counter(y_train).items())[0][1]} '
      f'records for the majority class and {sorted(Counter(y_train).items())[1][1]} '
      f'records for the minority class.')

print(f'The test data set has {sorted(Counter(y_test).items())[0][1]} '
      f'records for the majority class and {sorted(Counter(y_test).items())[1][1]} '
      f'records for the minority class.')
...
# Check the prediction loss threshold for 0.05% of outliers
