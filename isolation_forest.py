...
starttime = time.time()
isof = IsolationForest(n_estimators=1000, max_samples= 10000, contamination='auto', 
...
subsample = total_payments_if[total_payments_if['anomaly1'] == -1].sample(n=10, random_state=42)
subsample = subsample.append(total_payments_if[total_payments_if['anomaly1'] == 1].sample(n=10, random_state=42))

# get a subsample1 where all blocked vendor are inlcuded (blocked vendor is not 0)
subsample1 = total_payments_if[total_payments_if['Blocked_Vendor'] != 0].sample(n=125, random_state=42)
